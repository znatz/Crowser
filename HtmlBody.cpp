		...
		if (count == 0 ) 			// </tag> not existed
			{
				...
				if ( CountPart(Body,"<")<=1 ) 	   // < not existed
				...
				else       						   // < existed  , only friends existed
					 {
						...
						Root->LEndPos  = Body.Pos(">")+1;      	//
						Root->RStartPos = Search(Body,"<",2)-1; //
						return;
						// FRIEND ONLY
					 }
			}
		else if (count==1)			// </tag> existed once
			{
				...
				Root->LEndPos	= EPos(cBody,"<"+Root->TagString+">") + 1; //
				Root->RStartPos	= cBody.Pos(endTag)-1;   //
...
TAG * HtmlBody::AddNode(UnicodeString str)
{
	...
	TAG *newNode 	= new TAG;
	// !!!!!!!!!!!!!! TODO : the LEndPos is wrong.
	//newNode->LEndPos	=	cBody.Pos(str)+ str.Pos(">")+1;
	newNode->Next	= NULL;
	newNode->Friend	= NULL;
